...
def create_text_mask(text, img, x, y,
                     font):  # Some horrible stuff needed to workaround pillow's antialiasing which was messing up text colors and can't be disabled
    mask = Image.new("L", img.size, 0)
    mask_draw = ImageDraw.Draw(mask)
    mask_draw.text((x, y), text, font=font, fill=255,
                   align='center')  # Some more awfulness to work around more antialiasing issues. If everything is black lower the threshold
    threshold = 100
...
def compose_image(title, lower_line, length, position, scroll_pos_title=0, scroll_pos_lower=0, audio_level=0):
    ...
    if len(title) > 5:
        ...
        img.paste((0, 255, 0), mask=create_text_mask(title, img, (32 - len(title) * 6) // 2, -2,
                                                     font))  # The weird calculations are to center the text
    ...
    img.paste((0, 255, 255), mask=create_text_mask(f'{minutes:02}', img, 1, 16,
                                                   font))  # There are two draw calls so that is's perfectly centered. With one is would always be one pixel off
